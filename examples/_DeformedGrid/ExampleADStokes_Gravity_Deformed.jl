...
@views function main(nc, BC_template, D_template)
    ...
    phases.c[  inx_c, iny_c][(Xc.^2  .+  (Yc .+ 2.0).^2) .<= 0.5^2] .= 2
    phases.v[  inx_v, iny_v][(Xv.^2  .+  (Yv .+ 2.0).^2) .<= 0.5^2] .= 2
    phases.Vx[inx_Vx,iny_Vx][(Xvx.^2 .+ (Yvx .+ 2.0).^2) .<= 0.5^2] .= 2
    phases.Vy[inx_Vy,iny_Vy][(Xvy.^2 .+ (Yvy .+ 2.0).^2) .<= 0.5^2] .= 2
    ...
    for it=1:nt
        ...
        for iter=1:niter
            ...
            @timeit to "Residual" begin
                TangentOperator!(𝐷, 𝐷_ctl, τ, τ0, ε̇, λ̇, η, V,   Pt.c, ΔPt, Jinv, type, BC, materials, phases, Δ)
