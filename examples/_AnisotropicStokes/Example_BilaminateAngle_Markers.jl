...
@views function main(nc, layering, BC_template, D_template, factorization, η1 , η2, θ; useMarkers = false, nmpc)
    ...
    ε̇II     = ones(size_c...)
    ϕ       = zeros(2)
    SR = (xx=zeros(2), yy=zeros(2), xy=zeros(2), p=zeros(2))
    TAU= (xx=zeros(2), yy=zeros(2), xy=zeros(2), p=zeros(2))
    ...
    for it=1:nt
        ...
        # σ1.x   .= 1.0
        # σ1.y   .= 0.0
        # for i in eachindex(σ1.x)
        #     x = @SVector([σ1.x[i], σ1.y[i]])
        #     x′ = R* x
        #     σ1.x[i] = x′[1]
        #     σ1.y[i] = x′[2]
        # end
    ...
    return mean(τII[inner_x, inner_y]), ϕ[1], ϕ[2], SR, TAU
...
let
    ...
    for iθ in eachindex(θ)
        ...
        τ_w = @SMatrix([TAU.xx[1] TAU.xy[1]; TAU.xy[1] TAU.yy[1]])
        τ_s = @SMatrix([TAU.xx[2] TAU.xy[2]; TAU.xy[2] TAU.yy[2]])
        τ_w_rot = R * τ_w * R'
        τ_s_rot = R * τ_s * R'

        Exx_w[iθ] = E_w_rot[1,1]
        Exx_s[iθ] = E_s_rot[1,1]
        Eyy_w[iθ] = E_w_rot[2,2]
        Eyy_s[iθ] = E_s_rot[2,2]
        Exy_w[iθ] = E_w_rot[1,2]
        Exy_s[iθ] = E_s_rot[1,2]
        τxx_w[iθ] = τ_w_rot[1,1]
        τxx_s[iθ] = τ_s_rot[1,1]
        τyy_w[iθ] = τ_w_rot[2,2]
        τyy_s[iθ] = τ_s_rot[2,2]
        τxy_w[iθ] = τ_w_rot[1,2]
        τxy_s[iθ] = τ_s_rot[1,2]
        Pt_w[iθ]  = SR.p[1]
        Pt_s[iθ]  = SR.p[2]
        ...
        τxx_ff_w[iθ] = τff_w_rot[1,1]
        τyy_ff_w[iθ] = τff_w_rot[2,2]
        τxy_ff_w[iθ] = τff_w_rot[1,2]
        τxx_ff_s[iθ] = τff_s_rot[1,1]
        τyy_ff_s[iθ] = τff_s_rot[2,2]
    ...
    function Visualisation()
        ...
        cm.lines!(ax, θ*180/π, ϕ_ani*180/π, label=cm.L"$\phi^\text{T.I.}$", linestyle=:dash, color=:green )
        cm.lines!(ax, θ*180/π, S1_w_rot  *180/π, label=cm.L"$\phi_{w}^\text{pred}$", color=:pink )
        cm.lines!(ax, θ*180/π, S1_s_rot  *180/π, label=cm.L"$\phi_{s}^\text{pred}$", color=:blue )
        cm.scatter!(ax, θ*180/π, ϕ_w_lay  *180/π, label=cm.L"$\phi_w$", marker=:xcross, color=:pink  )
        cm.scatter!(ax, θ*180/π, ϕ_s_lay  *180/π, label=cm.L"$\phi_s$", marker=:xcross, color=:blue  )
        ...
        # cm.lines!(ax, θ*180/π, Pt_s, label=cm.L"$Pt_s$", linestyle=:dash )
        # cm.scatter!(ax, θ*180/π, Pt_s .+ Pt_w, label=cm.L"$x$" )
        # cm.axislegend(position=:rt, framevisible=false, labelsize=12, orientation=:horizontal, merge=true)
