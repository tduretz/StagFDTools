...
@views function main(BC_template, D_template)
    ...
    for it=1:nt
        ...
        #--------------------------------------------#
        # Average τ and ε̇ to centroids to be able to calc τ_II and ε̇_II
        τ̄xy = av2D(τ.xy)
        ε̇̄xy = av2D(ε̇.xy)
        τ_II  = sqrt.( 0.5.*(τ.xx[inx_c,iny_c].^2 + τ.yy[inx_c,iny_c].^2 + (-τ.xx[inx_c,iny_c]-τ.yy[inx_c,iny_c]).^2) .+ τ̄xy[inx_c,iny_c].^2 )
        ε̇_II  = sqrt.( 0.5.*(ε̇.xx[inx_c,iny_c].^2 + ε̇.yy[inx_c,iny_c].^2 + (-ε̇.xx[inx_c,iny_c]-ε̇.yy[inx_c,iny_c]).^2) .+ ε̇̄xy[inx_c,iny_c].^2 )
        #--------------------------------------------#
        ...
        p7 = heatmap(xc, yc,  τ.xx[inx_c,iny_c]', aspect_ratio=1, xlim=extrema(xc), title="τxx")
        p8 = heatmap(xc, yc,  τ.yy[inx_c,iny_c]', aspect_ratio=1, xlim=extrema(xc), title="τyy")
        p9 = heatmap(xc, yc,  ε̇.xx[inx_c,iny_c]', aspect_ratio=1, xlim=extrema(xc), title="ε̇xx")
        p10 = heatmap(xc, yc,  ε̇.yy[inx_c,iny_c]', aspect_ratio=1, xlim=extrema(xc), title="ε̇yy")
        p11 = heatmap(xv, yv,  τ.xy[inx_v,iny_v], aspect_ratio=1, xlim=extrema(xv), title="τxy")
        p12 = heatmap(xv, yv,  ε̇.xy[inx_v,iny_v], aspect_ratio=1, xlim=extrema(xv), title="ε̇xy")

        p13 = heatmap(xc, yc,  τ_II', aspect_ratio=1, xlim=extrema(xc), ylims=extrema(yc), title="τ_II")
        p14 = heatmap(xc, yc,  ε̇_II', aspect_ratio=1, xlim=extrema(xc), ylims=extrema(yc), title="ε̇_II")

        display(plot(p1, p2, p3, p4, layout=(2,2)))
        display(plot(p7,p8,p11, layout=(2,2)))      # stress components
        display(plot(p9,p10,p12, layout=(2,2)))     # strain rate components
        display(plot(p13,p14, layout=(1,2)))        # plots the second invariants
