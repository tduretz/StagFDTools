...
@views function main(BC_template, D_template)
    ...
    if uniform_grid
        ...
        if withghosts
            ...
            Δ = (x = zeros(nc.x+2), y = zeros(nc.x+2), t=fill(Δt0,1)) # nb cells
            Δ.x[2:end-1]   .= diff(xv)
            ...
            p1 = heatmap(xc, yc, volCell, aspect_ratio=1,  xlims=extrema(xv), title="Volume Cellules", color=:vik)
            volVertices = zeros(nc.x+1, nc.y+1)
            for i=1:nc.x+1
                for j=1:nc.y+1
                    volVertices[i,j] = ((Δ.x[i]+Δ.x[i+1])/2)*((Δ.y[j]+Δ.y[j+1])/2)
                end
            end
            p2 = heatmap(xv, yv, volVertices, aspect_ratio=1, xlims=extrema(xv), title="Volume Vertices", color=:vik)
    ...
    # # Mesh coordinates
    # grid = ( 
    #     c = ( Δ = ( x =  zeros(size_c...),  y =  zeros(size_c...)), ),
    #     v = ( Δ = ( x =  zeros(size_v...),  y =  zeros(size_v...)), ),
    # )
    # for j in iny_c
    #     grid.c.Δ.x[inx_c,j]   .= diff(xv)
    #     grid.c.Δ.x[[1,end],j] .= grid.c.Δ.x[[2,end-1],j]
    # end
    # for i in inx_c
    #     grid.c.Δ.y[i,iny_c]   .= diff(yv)
    #     grid.c.Δ.y[i,[1,end]] .= grid.c.Δ.y[i,[2,end-1]]
    # end
    # @show grid.c.Δ.x
    ...
    for it=1:nt
        ...
        p1 = heatmap(xv, yc, R.x[inx_Vx,iny_Vx]', aspect_ratio=1, xlim=extrema(xv), title="Vx", color=:vik)
        p2 = heatmap(xc, yv, R.y[inx_Vy,iny_Vy]', aspect_ratio=1, xlim=extrema(xc), title="Vy", color=:vik)
        p3 = heatmap(xc, yc, R.p[inx_c,iny_c]'.-mean( Pt[inx_c,iny_c]), aspect_ratio=1, xlim=extrema(xc), title="Pt", color=:vik)
