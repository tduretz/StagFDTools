...
function LocalRheology_var(ε̇, materials, phases, Δt)
    ...
    #display("avant avant avant")
    #display(isnan(ε̇[1]^2))
    #display(isnan(ε̇[2]^2))
    #display( isnan((ε̇[1]^2 + ε̇[2]^2 + (-ε̇[1]-ε̇[2])^2)/2 + ε̇[3]^2 ))
    P    = ε̇[4]
...
function LocalRheology_var(ε̇, Dkk, P0, materials, phases, Δt)
    ...
    λ̇ = 0.
    if materials.plasticity === :DruckerPrager
        τII, P, λ̇ = DruckerPrager(τII, P, ηvep, comp, β, Δt, C, cosϕ, sinϕ, sinψ, ηvp)
    elseif materials.plasticity === :tensile
        τII, P, λ̇ = Tensile(τII, P, ηvep, comp, β, Δt, materials.σT[phases], ηvp)
    elseif materials.plasticity === :Kiss2023
        σT   = materials.σT[phases]
        τII, P, λ̇ = Kiss2023(τII, P, ηvep, comp, β, Δt, C, ϕ, ψ, ηvp, materials.σT[phases], materials.δσT[phases], materials.P1[phases], materials.τ1[phases], materials.P2[phases], materials.τ2[phases])
    elseif materials.plasticity === :Hyperbolic
        model = Hyperbolic()
        σT   = materials.σT[phases]
        p = (C, cosϕ, sinϕ, cosψ, sinψ, σT, ηvp)
        τII, P, λ̇ = NonLinearReturnMapping(τII, P, ε̇II, Dkk, P0, ηvep, β, Δt, p, model)
...
function StressVector_var!(ε̇, Dkk, P0, materials, phases, Δt)
    η, λ̇, P, τII = LocalRheology_var(ε̇, Dkk, P0, materials, phases, Δt)
    τ       = @SVector([2 * η * ε̇[1],
                        2 * η * ε̇[2],
                        2 * η * ε̇[3],
                                  P])
