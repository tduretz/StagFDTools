...
function porosity_rate(Pt, Pf, Pt0, Pf0, ϕ0, p, Δt)
    ...
    dPtdt   = (Pt - Pt0) / Δt
    dPfdt   = (Pf - Pf0) / Δt
    return ((Pf - Pt)/χ + (dPtdt - dPfdt)/KΦ) * 1
end
...
function main_Havlin(nc)
    ...
    P_seafloor = 0*20e6
    Pf       .= P_seafloor .- p.ρl * p.gy .* Δy/2
    Pt       .= P_seafloor .- ρt   * p.gy .* Δy/2
    for j in (nc+2-1):-1:2
    ...
    for it=1:nt
       ...
       if mod(it, 100) == 0 || it==1
            ...
            @save "havlin_debug.jld2" Pt Pf τyy ϕ Vy Pt0 Pf0 τyy0 ϕ0
