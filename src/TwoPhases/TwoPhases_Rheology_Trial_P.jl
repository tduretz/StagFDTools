...
function TangentOperator!(𝐷, 𝐷_ctl, τ, τ0, ε̇, λ̇, η , V, P, ΔP, P0, Φ, Φ0, type, BC, materials, phases, Δ)
    ...
    for j=2:size(ε̇.xx,2)-1, i=2:size(ε̇.xx,1)-1
        ...
        ρfgC   = materials.ρf[phc] .* materials.g[2]
        ρfg    = SVector{2, Float64}(1/2 * (ρfgC[2:end-1,1:end-1] + ρfgC[2:end-1,2:end]) )
        ...
        kx_μ_xx = SVector{2, Float64}( @. (k_μ_xx[1:end-1,2:end-1] - k_μ_xx[2:end-0,2:end-1]) / 2 )
        k_μ_yy  = SMatrix{3,3, Float64}( @.  k_ηf0 * max.(Φ_loc, 1e-6).^n  )
        ky_μ_yy = SVector{2, Float64}( @. (k_μ_yy[2:end-1,1:end-1] - k_μ_yy[2:end-1,2:end-0]) / 2 )
        ∂Pf∂x   = SVector{2, Float64}( @. (Pf[2:end,2:end-1] - Pf[1:end-1,2:end-1] ) / Δ.x )
        ∂Pf∂y   = SVector{2, Float64}( @. (Pf[2:end-1,2:end] - Pf[2:end-1,1:end-1] ) / Δ.y )
    ...
    for j=3:size(ε̇.xy,2)-2, i=3:size(ε̇.xy,1)-2
        ...
        Pt0     = SMatrix{4,4}(     P0.t[ii,jj] for ii in i-2:i+1,   jj in j-2:j+1)
        Pf0     = SMatrix{4,4}(     P0.f[ii,jj] for ii in i-2:i+1,   jj in j-2:j+1)
        Pf      = SMatrix{4,4}(      P.f[ii,jj] for ii in i-2:i+1,   jj in j-2:j+1)
        Pt      = SMatrix{4,4}(      P.t[ii,jj] for ii in i-2:i+1,   jj in j-2:j+1)
        ...
        # Fluid density
        ρfgC   = materials.ρf[phc] .* materials.g[2]
        ρfg    = SMatrix{2, 3, Float64}(1/2 * (ρfgC[2:end-1,1:end-1] + ρfgC[2:end-1,2:end]) )

        # Set BCs
        Vx     = SetBCVx1(Vx, typex, bcx, Δ)
        Vy     = SetBCVy1(Vy, typey, bcy, Δ)
        Pf     = SetBCPf1(Pf, typepf, bcpf, Δ, ρfg)
        Pf0    = SetBCPf1(Pt0, typepf, bcpf, Δ, ρfg)

        # Porosity
        dPtdt   = SMatrix{4,4, Float64}( @. (Pt - Pt0) / Δt )
        dPfdt   = SMatrix{4,4, Float64}( @. (Pf - Pf0) / Δt )
        dΦdt    = SMatrix{4,4, Float64}( @. (dPfdt - dPtdt)/KΦ + (Pf - Pt)/ηΦ )
        if materials.linearizeϕ
        ...
        kx_μ_xx = SMatrix{3,2, Float64}( @. (k_μ_xx[1:end-1,2:end-1] - k_μ_xx[2:end-0,2:end-1]) / 2 )
        k_μ_yy  = SMatrix{4,4, Float64}( @.  k_ηf0 * max.(Φ_loc, 1e-6).^n  )
        ky_μ_yy = SMatrix{2,3, Float64}( @. (k_μ_yy[2:end-1,1:end-1] - k_μ_yy[2:end-1,2:end-0]) / 2 )
        ∂Pf∂x   = SMatrix{3,2, Float64}( @. (Pf[2:end,2:end-1] - Pf[1:end-1,2:end-1] ) / Δ.x )
        ∂Pf∂y   = SMatrix{2,3, Float64}( @. (Pf[2:end-1,2:end] - Pf[2:end-1,1:end-1] ) / Δ.y )
