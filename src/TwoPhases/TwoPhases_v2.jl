...
function AssembleFluidContinuity2D!(K, V, P, P0, ϕ, phases, materials, num, pattern, type, BC, nc, Δ) 
    ...
    for j in 1+shift.y:nc.y+shift.y, i in 1+shift.x:nc.x+shift.x
        ...
        ∂R∂Pt .= 0.
        ∂R∂Pf .= 0.
        autodiff(Enzyme.Reverse, FluidContinuity, Duplicated(Vx_loc, ∂R∂Vx), Duplicated(Vy_loc, ∂R∂Vy), Duplicated(Pt_loc, ∂R∂Pt), Const(P.t[i,j]), Duplicated(Pf_loc, ∂R∂Pf), Const(Pf0), Const(ϕ.c[i,j]), Const(phases.c[i,j]), Const(materials), Const(k_loc), Const(type_loc), Const(bcv_loc), Const(Δ))
