...
using Plots, StagFDTools.Rheology

line1_Dani(p,K,Δt,η_ve,η_vp,p1,t1)       = (η_ve + η_vp)/(Δt*K + 2.0/3.0*η_vp)*(-p + p1) + t1 
line2_Dani(p,K,Δt,η_ve,η_vp,p1,t1)       = (η_ve + η_vp)/(Δt*K + 2.0/3.0*η_vp)*(-p + p1) + t1 
line3_Dani(p,K,Δt,η_ve,η_vp,p1,t1,φ,ψ)   = (η_ve + η_vp)/((Δt*K + 2.0/3.0*η_vp)*sin(ψ))*(-p + p1) + t1  
...
function plastic_corrections!(τ, P, η_ve, η_vp, K, φ, ψ, C, σ_T, δσ_T, Δt, pc1, τc1, pc2, τc2)
    ε_dev_pl  = 0.0
    ε_vol_pl  = 0.0
    domain_pl = 0.0
...
function main() 
    ...
    l1    = line.(p_tr1, K, Δt, η_ve, π/2, pc1, τc1)
    l2    = line.(p_tr2, K, Δt, η_ve, π/2, pc2, τc2)
    l3    = line.(p_tr3, K, Δt, η_ve,   ψ, pc2, τc2)

    l1D    = line1_Dani.(p_tr1, K, Δt, η_ve, η_vp, pc1, τc1)
    l2D    = line2_Dani.(p_tr2, K, Δt, η_ve, η_vp, pc2, τc2)
    l3D    = line3_Dani.(p_tr3, K, Δt, η_ve, η_vp, pc2, τc2,φ,ψ)
    ...
    for i in eachindex(P_tr)
        @show "Trial state $(i)"
        τII[i],  P[i] = plastic_corrections!(τII_tr[i], P_tr[i], η_ve, η_vp, K, φ, ψ, C, σ_T, δσ_T, Δt, pc1, τc1, pc2, τc2)
        τII1[i], P1[i] = Kiss2023(τII_tr[i], P_tr[i], η_ve, comp, β, Δt, C, φ*180/π, ψ*180/π, η_vp, σ_T, δσ_T, pc1, τc1, pc2, τc2)
        p=plot!([P_tr[i], P[i]],[τII_tr[i], τII[i]])
        p=plot!([P_tr[i], P1[i]],[τII_tr[i], τII1[i]])
