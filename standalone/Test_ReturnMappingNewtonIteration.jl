function F_DP_hyperbolic_v1(x, P_trial, C, σT, cosΨ, sinΨ, ηvp)  
    ϵ  = -1e-13
    τ  = x[1] 
    ΔP = x[2]
    λ̇  = x[3]
...
function F_shear(x, τ_trial, P_trial, ε̇_eff, ηve, C, σT, cosΨ, sinΨ, ηvp)
    τ    = x[1]
    λ̇    = x[3]
    ∂Q∂σ = Enzyme.gradient(Enzyme.Forward, F_DP_hyperbolic_v1, x, Const(P_trial), Const(C), Const(σT), Const(cosΨ), Const(sinΨ), Const(ηvp))
...
function RheologyResidual(x, τ_trial, ε̇_eff, P_trial, Dkk, P0, ηve, K, Δt, C, σT, cosΨ, sinΨ, cosϕ, sinϕ, ηvp)
    return @SVector([
        F_shear(x, τ_trial, P_trial, ε̇_eff, ηve, C, σT, cosΨ, sinΨ, ηvp),
        F_vol(x, P_trial, Dkk, P0, K, Δt, C, σT, cosΨ, sinΨ, ηvp),
        F_DP_hyperbolic_v1(x, P_trial,  C, σT, cosϕ, sinϕ, ηvp),
...
let 
    ...
    P_ax_vp       = LinRange(-σT - ηvp*λ̇/sinϕ,  0.3/1e3, 100)
