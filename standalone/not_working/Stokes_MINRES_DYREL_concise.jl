...
@views function (@main)(nc)
    ...
    @show maximum(-G.x./D.x) 
    @show maximum(((2/dx) ./D.x) )
    ...
    for k in 1:max_iter
        ...
        if  mod(k, nout)==0
            ...
            λ0 = (dot(V.x, (1 ./D.x).*V.x) + dot(V.y, (1 ./D.y).*V.y) + dot(P, (1 ./D.p).*P)) / (dot(V.x, V.x) + dot(V.y, V.y) + dot(P, P)) 
            ...
            λ3  = abs( (dot(V̇.x, α.*(R.x .- R0.x)./D.x) + dot(V̇.y, α.*(R.y .- R0.y)./D.y) + dot(Ṗ, α.*(Rp .- Rp0)./D.p)) / ( dot(α.*V̇.x, α.*V̇.x) + dot(α.*V̇.y, α.*V̇.y) + dot(α.*Ṗ, α.*Ṗ)) )
            # @show λ0, λ1, λ2, λ3
